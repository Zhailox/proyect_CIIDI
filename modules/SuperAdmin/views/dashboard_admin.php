        ...
        <div style="flex-grow: 1; overflow-y: auto; max-height: 210px; display: flex; flex-direction: column; gap: 0.5rem;">
            ...
            <?php else: ?>
                <?php foreach ($ultimosLogs as $logItem): ?>
                    <?php 
                    $badgeColor = 'var(--texto-silenciado)';
                    ...
                    ?>
                    <div style="background: rgba(248, 250, 252, 0.8); border: 1px solid rgba(226, 232, 240, 0.8); border-radius: var(--radius-sm); padding: 6px 10px; font-size: 0.78rem;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2px;">
                            <span style="font-weight: 700; color: var(--texto-titulos);"><?= htmlspecialchars($logItem['accion'] ?? 'Acción') ?></span>
                            <span style="background: <?= $bgBadge ?>; color: <?= $badgeColor ?>; padding: 2px 8px; border-radius: var(--radius-sm); font-size: 0.7rem; font-weight: 700; border: 1px solid <?= $badgeColor ?>;"><?= htmlspecialchars($lvl) ?></span>
                        </div>
...
<div id="ag-core-health-modal-overlay" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(18, 26, 62, 0.65); backdrop-filter: blur(8px); z-index: 99999; align-items: center; justify-content: center; padding: 1.5rem;">
    <div class="sa-modal-card" style="width: 100%; max-width: 640px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25); overflow: hidden; animation: agModalFade 0.25s cubic-bezier(0.16, 1, 0.3, 1);">
        ...
        <div id="ag-core-health-body" style="padding: 1.6rem; max-height: 480px; overflow-y: auto;">
            <div style="text-align: center; padding: 2rem 0; color: var(--texto-silenciado);">
                <i class="ph-bold ph-spinner spin" style="font-size: 2.2rem; color: var(--color-secundario);"></i>
                <p style="margin-top: 0.8rem; font-weight: 700; color: var(--texto-titulos);">Ejecutando pings sintéticos al Kernel, BD y Módulos Core...</p>
            </div>
        </div>

        <div class="sa-modal-footer">
            <button type="button" onclick="cerrarTestCoreModal()" class="sa-btn-cancel">
                Cerrar Diagnóstico
            </button>
        </div>
    </div>
...
function ejecutarTestCore() {
    ...
    body.innerHTML = `
        <div style="text-align: center; padding: 2.5rem 0; color: var(--texto-silenciado);">
            <i class="ph-bold ph-spinner spin" style="font-size: 2.5rem; color: var(--color-secundario);"></i>
            <p style="margin-top: 1rem; font-weight: 800; color: var(--texto-titulos); font-size: 1rem;">Diagnosticando Respuesta del Core...</p>
            <p style="font-size: 0.82rem; color: var(--texto-silenciado); margin-top: 0.2rem;">Evaluando latencia de BD, constantes de Kernel y permisos de storage.</p>
        </div>
    `;
    ...
    .then(data => {
        ...
        if (data.pruebas && Array.isArray(data.pruebas)) {
            data.pruebas.forEach(item => {
                ...
                const bgIcon = esOk ? 'rgba(112, 144, 203, 0.15)' : 'rgba(18, 26, 62, 0.12)';
                const colorIcon = esOk ? 'var(--color-terciario)' : 'var(--color-principal)';
                const iconClass = esOk ? 'ph-check-circle' : 'ph-x-circle';

                htmlPruebas += `
                    <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: var(--radius-sm); padding: 1rem; margin-bottom: 0.8rem; display: flex; align-items: flex-start; gap: 12px;">
                        <div style="width: 32px; height: 32px; border-radius: var(--radius-sm); background: ${bgIcon}; color: ${colorIcon}; display: flex; align-items: center; justify-content: center; font-size: 1.2rem; flex-shrink: 0; margin-top: 2px;">
                            <i class="ph-bold ${iconClass}"></i>
                        </div>
                        <div style="flex: 1;">
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                <h4 style="margin: 0; font-size: 0.92rem; font-weight: 800; color: var(--texto-titulos);">${item.modulo}</h4>
                                <span style="font-size: 0.75rem; font-weight: 800; background: ${bgIcon}; color: ${colorIcon}; padding: 2px 8px; border-radius: var(--radius-sm); border: 1px solid ${colorIcon};">${item.info}</span>
                            </div>
                            <p style="margin: 0.3rem 0 0 0; font-size: 0.82rem; color: var(--texto-silenciado);">${item.detalles}</p>
                        </div>
                    </div>
                `;
            });
        ...
        body.innerHTML = `
            <div style="background: rgba(112, 144, 203, 0.08); border: 1px solid rgba(112, 144, 203, 0.25); border-radius: var(--radius-sm); padding: 1.1rem; margin-bottom: 1.2rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.8rem;">
                <div>
                    <span style="background: ${headerColor}; color: #ffffff; font-size: 0.75rem; font-weight: 800; padding: 4px 10px; border-radius: var(--radius-sm); text-transform: uppercase;">
                        ${statusBadge}
                    </span>
                    <h4 style="margin: 0.5rem 0 0 0; font-size: 1.1rem; font-weight: 800; color: var(--texto-titulos);">
                        Respuesta del Core: ${data.duracion_ms} ms
                    </h4>
                </div>
                <div style="text-align: right; font-size: 0.82rem; color: var(--texto-silenciado);">
                    <div><strong>PHP Version:</strong> ${data.php_version}</div>
                    <div><strong>Memoria en Uso:</strong> ${data.memoria_mb} MB</div>
                </div>
            </div>

            <h4 style="margin: 0 0 0.8rem 0; font-size: 0.88rem; font-weight: 800; color: var(--texto-titulos); text-transform: uppercase; letter-spacing: 0.5px;">Desglose de Diagnóstico Sintáctico</h4>
            ${htmlPruebas}
        `;
    })
    .catch(err => {
        body.innerHTML = `
            <div class="sa-alert sa-alert-error" style="margin-bottom: 0;">
